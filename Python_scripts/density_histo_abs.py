...
def main(taskname):
    ...
    for i in range(3):
        for j in range(3):
            ax = axs[i, j]
            fast_Js = all_Js_abs['all_Js_abs'][i, j, 0].flatten()
            slow_Js = all_Js_abs['all_Js_abs'][i, j, 1].flatten()
            
            # Create 2D histogram
            hist, xedges, yedges = np.histogram2d(fast_Js, slow_Js, bins=50, range=[[0, 2], [0, 2]])
            ...
            ax_hist_y.hist(slow_Js, bins=50, range=(0, 2), color='black', orientation='horizontal')
    ...
    plt.savefig('figures/kernal_comp/kernel_comp_density_abs_' + taskname + '.png', dpi=200)
    # plt.show()
