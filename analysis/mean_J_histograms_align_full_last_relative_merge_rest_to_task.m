...
for kernel_idx = 1:n_conn_kernel
        ...
        for i = 1:n_area
            ...
            for session_type_idx = 1:2
                for align_idx = 1:1
                    for prepost_idx = 1:2
                        for posneg_idx = 1:2
                            bar_height_task = all_all_histogram_data(session_type_idx, i, j, kernel_idx, posneg_idx, prepost_idx, 1, align_idx);
                            bar_height_close = all_all_histogram_data(session_type_idx, i, j, kernel_idx, posneg_idx, prepost_idx, 3, align_idx);
                            % relative_diff = (bar_height_post - bar_height_pre) ./ bar_height_pre;
                            relative_diff = bar_height_close ./ bar_height_task;
                            bar_pos = prepost_idx + (session_type_idx-1.5) * 0.35 + (posneg_idx - 1.5) * 0.15;
            ...
            if strcmp(plot_mode, 'prepost')
                ...
                ylabel('eye close/task');
                for prepost_idx = 1:2
                    for session_type_idx = 1:2
                        for posneg_idx = 1:2
                            k = (prepost_idx-1)*4 + (session_type_idx-1)*2 + posneg_idx;
    ...
    saveas(f, [fig_folder, '/J_histograms_restToTask_kernel', num2str(kernel_idx), '.png']);
end
