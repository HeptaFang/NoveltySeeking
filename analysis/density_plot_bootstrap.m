...
for kernel_idx = 1:n_conn_kernel
    ...
    all_data = cell(3, 3, 3, n_states, 3, 2); % (i, j, prepost, state, align, session_type)
    all_error = cell(3, 3, 3, n_states, 3, 2); % (i, j, prepost, state, align, session_type)
    for session_type_idx = 1:2
        session_type = session_types{session_type_idx};
        if strcmp(session_type, 'Muscimol')
            n_session = 10;
                ...
                for prepost_idx = 1:3
                    ...
                    for state_idx = 1:n_states
                        for align_idx = 1:n_aligns
                            ...
                            all_data{i, j, prepost_idx, state_idx, align_idx, session_type_idx} = data;
                            all_error{i, j, prepost_idx, state_idx, align_idx, session_type_idx} = error;
                        end
                    end
                end
            end
    ...
    for state_idx = 1:n_states
        for align_idx = 1:n_aligns
            f = figure("Visible", "off", 'PaperPosition', [0 0 30 15]);
