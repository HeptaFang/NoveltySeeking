    ...
    for state_idx = 1:n_states
        ...
        for i = 1:n_area
            for j = 1:n_area
                ...
                for grid_i = 1:3
                    for grid_j = 1:3
                        a11 = data_grid_raw(grid_i, grid_j);
                        a12 = sum(data_grid_raw(grid_i, :));
                        a21 = sum(data_grid_raw(:, grid_j));
                        a22 = sum(data_grid_raw(:));
                        if (a11) == 0
                            odd_ratio = 0;
                        else
        ...
        title(tiles, [session_type,', significant count grid, Kernel ', num2str(kernel_idx), ', ', states{state_idx}]);
