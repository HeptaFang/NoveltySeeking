...
for reg_idx = 1:length(regressors)
    for stage_idx = 1:length(stages)
        for i = 1:1
            ...
            k = mod(n, 2);
            n = floor(n / 2);
            all_options{end}.use_abs_beta = logical(k);
            k = mod(n, 2);
            n = floor(n / 2);
            all_options{end}.use_abs_J = logical(k);
    ...
    for session_type_idx = 1:2
        ...
        for kernel_idx = 1:n_conn_kernel
            for state_idx = 1:n_states
                ...
                distribution_data = cell(2, 4); % distribution data over beta product. (pre/post, area type). each cell: .dist (L, sig/nsig) .edge(L+1, 1)
                
                beta_product_mean = zeros(2, 2, n_area_types); % (pre/post, sig/n-sig, area type)
                beta_product_se = zeros(2, 2, n_area_types); % (pre/post, sig/n-sig, area type)
                ...
                for area_type_idx = 1:n_area_types  % ACC, VLPFC, across, within
                    ...
                    for prepost_idx = 1:2
                        ...
                        for session_idx = used_session
                            switch area_type
                                case 'ACC'
                                    ...
                                    J_pre = J_data{1, 1, kernel_idx, state_idx, 1, session_idx, 1, session_type_idx}; % (n, n)
                                    J_post = J_data{1, 1, kernel_idx, state_idx, 1, session_idx, 2, session_type_idx}; % (n, n)
                                    J_diff = J_post - J_pre; % (n, n)
                                    J_err_pre = J_data_err{1, 1, kernel_idx, state_idx, 1, session_idx, 1, session_type_idx}; % (n, n)
                                    J_err_post = J_data_err{1, 1, kernel_idx, state_idx, 1, session_idx, 2, session_type_idx}; % (n, n)
                                    J_err_diff = sqrt(J_err_pre.^2 + J_err_post.^2); % (n, n)
                                    mask = ~eye(size(beta_product)); % non-diagonal elements

                                    X = [X; beta_product(mask)];
                                    Y = [Y; J_diff(mask)];
                                    sig_X = [sig_X; tuning_filter(mask)];
                                    sig_Y = [sig_Y; abs(J_diff(mask)) > filter_threshold * J_err_diff(mask)];
                                ...
                                case 'VLPFC'
                                    ...
                                    J_pre = J_data{3, 3, kernel_idx, state_idx, 1, session_idx, 1, session_type_idx}; % (n, n)
                                    J_post = J_data{3, 3, kernel_idx, state_idx, 1, session_idx, 2, session_type_idx}; % (n, n)
                                    J_diff = J_post - J_pre; % (n, n)
                                    J_err_pre = J_data_err{3, 3, kernel_idx, state_idx, 1, session_idx, 1, session_type_idx}; % (n, n)
                                    J_err_post = J_data_err{3, 3, kernel_idx, state_idx, 1, session_idx, 2, session_type_idx}; % (n, n)
                                    J_err_diff = sqrt(J_err_pre.^2 + J_err_post.^2); % (n, n)
                                    mask = ~eye(size(beta_product)); % non-diagonal elements
                                ...
                                case 'Across area'
                                    ...
                                    J_pre = J_data{1, 3, kernel_idx, state_idx, 1, session_idx, 1, session_type_idx}; % (n, n)
                                    J_post = J_data{1, 3, kernel_idx, state_idx, 1, session_idx, 2, session_type_idx}; % (n, n)
                                    J_diff = J_post - J_pre; % (n, n)
                                    J_err_pre = J_data_err{1, 3, kernel_idx, state_idx, 1, session_idx, 1, session_type_idx}; % (n, n)
                                    J_err_post = J_data_err{1, 3, kernel_idx, state_idx, 1, session_idx, 2, session_type_idx}; % (n, n)
                                    J_err_diff = sqrt(J_err_pre.^2 + J_err_post.^2); % (n, n)
                                    assert(all(size(J_diff) == size(beta_product)), 'J and beta product size mismatch');
                                    X = [X; beta_product(:)];
                                    Y = [Y; J_diff(:)];
                                    sig_X = [sig_X; tuning_filter(:)];
                                    sig_Y = [sig_Y; abs(J_diff(:)) > filter_threshold * J_err_diff(:)];
                                    if strcmp(beta_method, 'product')
                                    ...
                                    J_pre = J_data{3, 1, kernel_idx, state_idx, 1, session_idx, 1, session_type_idx}; % (n, n)
                                    J_post = J_data{3, 1, kernel_idx, state_idx, 1, session_idx, 2, session_type_idx}; % (n, n)
                                    J_diff = J_post - J_pre; % (n, n)
                                    J_err_pre = J_data_err{3, 1, kernel_idx, state_idx, 1, session_idx, 1, session_type_idx}; % (n, n)
                                    J_err_post = J_data_err{3, 1, kernel_idx, state_idx, 1, session_idx, 2, session_type_idx}; % (n, n)
                                    J_err_diff = sqrt(J_err_pre.^2 + J_err_post.^2); % (n, n)
                                    assert(all(size(J_diff) == size(beta_product)), 'J and beta product size mismatch');
                                    X = [X; beta_product(:)];
                                    Y = [Y; J_diff(:)];
                        ...
                        % scatter plot
                        % axis equal; 
                        scatter(X(~sig_Y), Y(~sig_Y), 4, 'b', 'filled', 'MarkerFaceAlpha', 0.5, 'MarkerEdgeColor', 'none');
                        scatter(X(sig_Y), Y(sig_Y), 4, 'r', 'filled', 'MarkerFaceAlpha', 0.5, 'MarkerEdgeColor', 'none');
                        ...
                        % show stats
                        text(0.6, 0.9, ['n = ', num2str(length(X))], 'Units', 'normalized', 'FontSize', 8, 'Color', 'k');
                        text(0.6, 0.82, ['R = ', num2str(R(1, 2), '%.3f')], 'Units', 'normalized', 'FontSize', 8, 'Color', 'k');
                        text(0.6, 0.74, ['p = ', num2str(P(1, 2), '%.3f')], 'Units', 'normalized', 'FontSize', 8, 'Color', 'k');
                        text(0.6, 0.66, ['y = ', num2str(b, '%.3f'), '+', num2str(k, '%.3f'), 'x'], 'Units', 'normalized', 'FontSize', 8, 'Color', 'k');
                        ...
                        [~, edges] = histcounts(X, 50);
                        dist_sig = histcounts(X(sig_Y), edges);
                        dist_nsig = histcounts(X(~sig_Y), edges);
                        distribution_data{prepost_idx, area_type_idx}.dist = [dist_sig', dist_nsig'];
                        distribution_data{prepost_idx, area_type_idx}.edge = edges;

                        % save beta product mean and std
                        beta_product_mean(prepost_idx, 1, area_type_idx) = mean(X(sig_Y));
                        beta_product_mean(prepost_idx, 2, area_type_idx) = mean(X(~sig_Y));
                        beta_product_se(prepost_idx, 1, area_type_idx) = std(X(sig_Y))/sqrt(sum(sig_Y));
                        beta_product_se(prepost_idx, 2, area_type_idx) = std(X(~sig_Y))/sqrt(sum(~sig_Y));
                    end
                ...
                % save figure
                file_folder = [root_path, 'figures/GLM/bootstrap/Jdiff/', switches,'/',];
                ...
                % plot distribution of beta product
                f = figure('Position', [100, 100, 1200, n_area_types*400], 'Visible', 'off');
                t = tiledlayout(n_area_types, 2, 'Padding', 'compact', 'TileSpacing', 'compact');
                ...
                for area_type_idx = 1:n_area_types
                    for prepost_idx = 1:2
                        nexttile;
                        hold on;
                        edges = distribution_data{prepost_idx, area_type_idx}.edge;
                        dist = distribution_data{prepost_idx, area_type_idx}.dist;
                        dist_sig = dist(:, 1);
                        dist_nsig = dist(:, 2);
                        log_dist_sig = log1p(dist_sig); % log scale
                        log_dist_nsig = log1p(dist_nsig); % log scale
                        norm_log_dist_sig = log_dist_sig / sum(log_dist_sig); % normalize
                        norm_log_dist_nsig = log_dist_nsig / sum(log_dist_nsig); % normalize
                        mean_sig = beta_product_mean(prepost_idx, 1, area_type_idx);
                        mean_nsig = beta_product_mean(prepost_idx, 2, area_type_idx);
                        se_sig = beta_product_se(prepost_idx, 1, area_type_idx);
                        se_nsig = beta_product_se(prepost_idx, 2, area_type_idx);
                        % histogram red: significant, blue: non-significant, alpha=0.5
                        % histogram('BinEdges', edges, 'BinCounts', norm_log_dist_sig, 'FaceColor', [1, 0, 0], 'EdgeColor', 'none', 'FaceAlpha', 0.3);
                        % histogram('BinEdges', edges, 'BinCounts', norm_log_dist_nsig, 'FaceColor', [0, 0, 1], 'EdgeColor', 'none', 'FaceAlpha', 0.3);
                        histogram('BinEdges', edges, 'BinCounts', norm_log_dist_sig, 'FaceColor', [1, 0, 0], 'FaceAlpha', 0.3);
                        histogram('BinEdges', edges, 'BinCounts', norm_log_dist_nsig, 'FaceColor', [0, 0, 1], 'FaceAlpha', 0.3);
                        legend({'Sig', 'Non-sig'}, 'Location', 'northeast', 'FontSize', 12);
                        % plot mean and se
                        plot([mean_sig, mean_sig], ylim, 'r--', 'LineWidth', 1);
                        plot([mean_nsig, mean_nsig], ylim, 'b--', 'LineWidth', 1);
                        % plot se
                        plot([mean_sig-se_sig, mean_sig+se_sig], [0.95*max(ylim), 0.95*max(ylim)], 'r-', 'LineWidth', 1);
                        plot([mean_nsig-se_nsig, mean_nsig+se_nsig], [0.925*max(ylim), 0.925*max(ylim)], 'b-', 'LineWidth', 1);

                        hold off;
                        ylabel('Normalized log density');
                        xlabel('Beta product');
                        title([prepost_names{prepost_idx}, ', ', area_type_names{area_type_idx}]);
                        % set(gca, 'XTick', linspace(edges(1), edges(end), 5), 'FontSize', 12);
                    end
                end

                file_name = [file_folder, 'TuningCorr_dist_', session_type, '_', state, '_', num2str(kernel_idx), '.png'];
                saveas(f, file_name);
                close(f);

                % % plot beta product mean and std
                % % PLOT method 1: pre and post
                % % subplot: 1 colume and 2 rows: pre and post
                % f = figure('Position', [100, 100, 400, 800], 'Visible', 'off');
                % t = tiledlayout(2, 1, 'Padding', 'compact', 'TileSpacing', 'compact');
                % title(t, [session_type, ', Kernel ', num2str(kernel_idx), ', ', state]);
                % ax1 = nexttile;
                % % x axis: area type, y axis: beta product mean. 2 bars for significant and non-significant
                % hold on;
                % for area_type_idx = 1:n_area_types
                %     bar(area_type_idx-0.2, beta_product_mean(1, 1, area_type_idx), 0.4, 'FaceColor', [0.4, 0.4, 0.4], 'EdgeColor', 'k', 'LineWidth', 1);
                %     bar(area_type_idx+0.2, beta_product_mean(1, 2, area_type_idx), 0.4, 'FaceColor', [0.85, 0.85, 0.85], 'EdgeColor', 'k', 'LineWidth', 1);
                %     errorbar(area_type_idx-0.2, beta_product_mean(1, 1, area_type_idx), beta_product_se(1, 1, area_type_idx), 'k', 'LineStyle', 'none', 'LineWidth', 1);
                %     errorbar(area_type_idx+0.2, beta_product_mean(1, 2, area_type_idx), beta_product_se(1, 2, area_type_idx), 'k', 'LineStyle', 'none', 'LineWidth', 1);
                % end
                % hold off;
                % ylabel('Beta product mean (pre)');
                % legend({'Sig', 'Non-sig'}, 'Location', 'northwest', 'FontSize', 12, 'Box', 'off');
                % set(gca, 'XTick', 1:n_area_types, 'XTickLabel', area_type_names, 'FontSize', 12);
                % xlim([0.5, n_area_types+0.5]);
                % max_ylim = ylim;
                % ax2=nexttile;
                % hold on;
                % for area_type_idx = 1:n_area_types
                %     bar(area_type_idx-0.2, beta_product_mean(2, 1, area_type_idx), 0.4, 'FaceColor', [0.4, 0.4, 0.4], 'EdgeColor', 'k', 'LineWidth', 1);
                %     bar(area_type_idx+0.2, beta_product_mean(2, 2, area_type_idx), 0.4, 'FaceColor', [0.85, 0.85, 0.85], 'EdgeColor', 'k', 'LineWidth', 1);
                %     errorbar(area_type_idx-0.2, beta_product_mean(2, 1, area_type_idx), beta_product_se(2, 1, area_type_idx), 'k', 'LineStyle', 'none', 'LineWidth', 1);
                %     errorbar(area_type_idx+0.2, beta_product_mean(2, 2, area_type_idx), beta_product_se(2, 2, area_type_idx), 'k', 'LineStyle', 'none', 'LineWidth', 1);
                % end
                % hold off;
                % ylabel('Beta product mean (post)');
                % set(gca, 'XTick', 1:n_area_types, 'XTickLabel', area_type_names, 'FontSize', 12);
                % xlim([0.5, n_area_types+0.5]);
                ...
                % PLOT method 2: each area type in a separate subplot
                f = figure('Position', [100, 100, 800, 800], 'Visible', 'off');
                t = tiledlayout(2, 2, 'Padding', 'compact', 'TileSpacing', 'compact');
                title(t, [session_type, ', Kernel ', num2str(kernel_idx), ', ', state]);

                diff_all = zeros(n_area_types, 2); % sig/non-sig
                diff_se_all = zeros(n_area_types, 2); % sig/non-sig

                for area_type_idx = 1:n_area_types
                    nexttile;
                    hold on;
                    pre_sig = beta_product_mean(1, 1, area_type_idx);
                    pre_non_sig = beta_product_mean(1, 2, area_type_idx);
                    post_sig = beta_product_mean(2, 1, area_type_idx);
                    post_non_sig = beta_product_mean(2, 2, area_type_idx);
                    pre_sig_se = beta_product_se(1, 1, area_type_idx);
                    pre_non_sig_se = beta_product_se(1, 2, area_type_idx);
                    post_sig_se = beta_product_se(2, 1, area_type_idx);
                    post_non_sig_se = beta_product_se(2, 2, area_type_idx);
                    % bar plot: x: pre/post, color: significant/non-significant
                    bar(1-0.2, pre_sig, 0.4, 'FaceColor', [0.4, 0.4, 0.4], 'EdgeColor', 'k', 'LineWidth', 1);
                    bar(1+0.2, pre_non_sig, 0.4, 'FaceColor', [0.85, 0.85, 0.85], 'EdgeColor', 'k', 'LineWidth', 1);
                    bar(2-0.2, post_sig, 0.4, 'FaceColor', [0.4, 0.4, 0.4], 'EdgeColor', 'k', 'LineWidth', 1);
                    bar(2+0.2, post_non_sig, 0.4, 'FaceColor', [0.85, 0.85, 0.85], 'EdgeColor', 'k', 'LineWidth', 1);
                    errorbar(1-0.2, pre_sig, pre_sig_se, 'k', 'LineStyle', 'none', 'LineWidth', 1);
                    errorbar(1+0.2, pre_non_sig, pre_non_sig_se, 'k', 'LineStyle', 'none', 'LineWidth', 1);
                    errorbar(2-0.2, post_sig, post_sig_se, 'k', 'LineStyle', 'none', 'LineWidth', 1);
                    errorbar(2+0.2, post_non_sig, post_non_sig_se, 'k', 'LineStyle', 'none', 'LineWidth', 1);
                    % relative difference
                    rel_diff_sig = (post_sig - pre_sig) / max(abs([pre_sig, post_sig]));
                    rel_diff_non_sig = (post_non_sig - pre_non_sig) / max(abs([pre_non_sig, post_non_sig]));
                    rel_diff_sig_se = sqrt((post_sig_se^2 + pre_sig_se^2) / max(abs([pre_sig, post_sig]))^2);
                    rel_diff_non_sig_se = sqrt((post_non_sig_se^2 + pre_non_sig_se^2) / max(abs([pre_non_sig, post_non_sig]))^2);
                    % % plot relative difference
                    % bar(3-0.2, rel_diff_sig, 0.4, 'FaceColor', [0.4, 0.4, 0.4], 'EdgeColor', 'k', 'LineWidth', 1);
                    % bar(3+0.2, rel_diff_non_sig, 0.4, 'FaceColor', [0.85, 0.85, 0.85], 'EdgeColor', 'k', 'LineWidth', 1);
                    % errorbar(3-0.2, rel_diff_sig, rel_diff_sig_se, 'k', 'LineStyle', 'none', 'LineWidth', 1);
                    % errorbar(3+0.2, rel_diff_non_sig, rel_diff_non_sig_se, 'k', 'LineStyle', 'none', 'LineWidth', 1);
                    diff_all(area_type_idx, 1) = rel_diff_sig;
                    diff_all(area_type_idx, 2) = rel_diff_non_sig;
                    diff_se_all(area_type_idx, 1) = rel_diff_sig_se;
                    diff_se_all(area_type_idx, 2) = rel_diff_non_sig_se;
                    
                    hold off;
                    ylabel('Beta product mean');
                    title(area_type_names{area_type_idx});
                    set(gca, 'XTick', [1, 2], 'XTickLabel', {'Pre', 'Post'}, 'FontSize', 12);
                    xlim([0.5, 2.5]);
                    if area_type_idx == 2
                        legend({'Sig', 'Non-sig'}, 'Location', 'northeast', 'FontSize', 12, 'Box', 'off');
                    end
                end

                % save figure
                file_name = [file_folder, 'TuningCorr_beta_product_', session_type, '_', state, '_', num2str(kernel_idx), '.png'];
                saveas(f, file_name);
                close(f);

                % plot relative difference
                f = figure('Position', [100, 100, 400, 400], 'Visible', 'off');
                t = tiledlayout(1, 1, 'Padding', 'compact', 'TileSpacing', 'compact');
                title(t, [session_type, ', Kernel ', num2str(kernel_idx), ', ', state]);
                hold on;
                for area_type_idx = 1:n_area_types
                    bar(area_type_idx-0.2, diff_all(area_type_idx, 1), 0.4, 'FaceColor', [0.4, 0.4, 0.4], 'EdgeColor', 'k', 'LineWidth', 1);
                    bar(area_type_idx+0.2, diff_all(area_type_idx, 2), 0.4, 'FaceColor', [0.85, 0.85, 0.85], 'EdgeColor', 'k', 'LineWidth', 1);
                    errorbar(area_type_idx-0.2, diff_all(area_type_idx, 1), diff_se_all(area_type_idx, 1), 'k', 'LineStyle', 'none', 'LineWidth', 1);
                    errorbar(area_type_idx+0.2, diff_all(area_type_idx, 2), diff_se_all(area_type_idx, 2), 'k', 'LineStyle', 'none', 'LineWidth', 1);
                end
                hold off;
                ...
                legend({'Sig', 'Non-sig'}, 'Location', 'northwest', 'FontSize', 12, 'Box', 'off');
                % save figure
                file_name = [file_folder, 'TuningCorr_beta_product_diff_', session_type, '_', state, '_', num2str(kernel_idx), '.png'];
                saveas(f, file_name);
                close(f);
