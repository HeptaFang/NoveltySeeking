...
for kernel_idx = 1:n_conn_kernel
    ...
    for area_type_idx = 3:4 % ACC, VLPFC, across area, within_area
        % connectivity change index
        nexttile(t, (area_type_idx-3)*2+1);
        ...
        CI_data = cell(3, n_states, 2); % bootstrap CI, muscimol/saline/corrected, state, pos/neg
        ...
        for state_idx = 1:n_states
            ...
            for session_type_idx = 1:2
                ...
                sig_diff = (sig_ratio(:, 2) - sig_ratio(:, 1)) ./ (sig_ratio(:, 2) + sig_ratio(:, 1)); % pos/neg
                sig_diff_error = sqrt((sig_error(:, 2).^2 + sig_error(:, 1).^2) ./ (sig_ratio(:, 2) + sig_ratio(:, 1)).^2); % pos/neg
            ...
            for i=1:bootstrap_N
                % bootstrap sampling. M: significant connection, N: total connection
                % resampling is equivalent to drawing from a binomial distribution with p = M/N
                ...
                % connectivity change index
                muscimol_ratios(i, 1) = (nnz(mus_post_pos)-nnz(mus_pre_pos))/(nnz(mus_post_pos)+nnz(mus_pre_pos));
                muscimol_ratios(i, 2) = (nnz(mus_post_neg)-nnz(mus_pre_neg))/(nnz(mus_post_neg)+nnz(mus_pre_neg));
                saline_ratios(i, 1) = (nnz(sal_post_pos)-nnz(sal_pre_pos))/(nnz(sal_post_pos)+nnz(sal_pre_pos));
                saline_ratios(i, 2) = (nnz(sal_post_neg)-nnz(sal_pre_neg))/(nnz(sal_post_neg)+nnz(sal_pre_neg));
            end
            ...
            CI_data{3, state_idx, 1} = prctile(ratio_diff(:, 1), [2.5, 97.5]);
            CI_data{3, state_idx, 2} = prctile(ratio_diff(:, 2), [2.5, 97.5]);
        ...
        % plot connectivity change index
        hold on;
        % four hidden bar plots for legend: Muscimol positive, Muscimol negative, Saline positive, Saline negative
        bar(0, 0, 0, 'FaceColor', 'r', 'EdgeColor', 'k', 'FaceAlpha', 1); % dummy bar for Muscimol positive
        bar(0, 0, 0, 'FaceColor', 'b', 'EdgeColor', 'k', 'FaceAlpha', 1); % dummy bar for Muscimol negative
        bar(0, 0, 0, 'FaceColor', 'r', 'EdgeColor', 'k', 'FaceAlpha', 0.3); % dummy bar for Saline positive
        bar(0, 0, 0, 'FaceColor', 'b', 'EdgeColor', 'k', 'FaceAlpha', 0.3); % dummy bar for Saline negative
        ...
        for state_idx = 1:n_states
            for posneg_idx = 1:2
                for session_type_idx = 1:2
                    ...
                    if CI(2) > global_max_y
                        global_max_y = CI(2);
                    end
                end
            end
        end
        y_lim_max = global_max_y*1.2;

        for state_idx = 1:n_states
            for posneg_idx = 1:2
                max_CI = 0;
                for session_type_idx = 1:2
                    x = state_idx + (session_type_idx-1.5)*0.15 + (posneg_idx-1.5)*0.35;
                    ...
                    b = bar(x, y, 0.15, 'FaceColor', color, 'EdgeColor', 'k', 'FaceAlpha', alpha);
                    ...
                    errorbar(x, CI_center, CI_width, 'k', 'LineStyle', 'none', 'LineWidth', 0.8); % CI
                    ...
                    if p < 0.05
                        ...
                        % text(x, y + y_err + 0.02, sig_str, 'FontSize', 6, 'Color', 'r', 'HorizontalAlignment', 'center');
                        text(x, CI(2) + 0.01*y_lim_max, sig_str, 'FontSize', 6, 'Color', 'r', 'HorizontalAlignment', 'center');
                ...
                if p < 0.05
                    ...
                    x_pos = state_idx + (posneg_idx-1.5)*0.35;
                    y_pos = max_CI + 0.06*y_lim_max;
                    % y_pos = y_lim_max*(1.1/1.2);
                    text(x_pos, y_pos, sig_str, 'FontSize', 6, 'Color', 'k', 'HorizontalAlignment', 'center');
                    % plot a line between the two bars
                    x_pos = [x_pos-0.075, x_pos+0.075];
                    y_pos = [y_pos-0.01*y_lim_max, y_pos-0.01*y_lim_max];
        ...
        % ylim([0, global_max_y*1.4]);
        ylim([-1, 1]);
        ylabel('Connectivity diff index');

        set(gca, 'FontSize', 8);

        title([area_type_names{area_type_idx}, ', pre-post ratio']);
        % legends = {'Muscimol positive', '', 'Muscimol negative', '', 'Saline positive', '', 'Saline negative', ''};
        legends = {'Muscimol positive', 'Muscimol negative', 'Saline positive', 'Saline negative'};
        % 2x2 legend, small
        legend(legends, 'Location', 'south', 'NumColumns', 2, 'FontSize', 6, 'Box', 'off', 'Orientation', 'horizontal');

        % plot relative change index = muscimol/saline
        nexttile(t, (area_type_idx-3)*2+2);
        plot_data = plot_data(1, :, :) - plot_data(2, :, :); % state, pos/neg
        plot_data = reshape(plot_data, n_states, 2);
        CI_data = CI_data(3, :, :);
        CI_data = reshape(CI_data, n_states, 2);

        global_max_y = 0;
        for state_idx = 1:n_states
            for posneg_idx = 1:2
                CI = CI_data{state_idx, posneg_idx};
                if CI(2) > global_max_y
                    global_max_y = CI(2);
                end
            end
        ...
        hold on;
        bar(0, 0, 0, 'FaceColor', 'r', 'EdgeColor', 'k', 'FaceAlpha', 1); % dummy bar for Positive
        bar(0, 0, 0, 'FaceColor', 'b', 'EdgeColor', 'k', 'FaceAlpha', 1); % dummy bar for Negative
        for state_idx = 1:n_states
            for posneg_idx = 1:2
                x = state_idx + (posneg_idx-1.5)*0.25;
                y = plot_data(state_idx, posneg_idx);
                y_err = plot_data_err(1, state_idx, posneg_idx) - plot_data_err(2, state_idx, posneg_idx);
                if posneg_idx == 1
                    color = 'r';
                else
                    color = 'b';
                ...
                b = bar(x, y, 0.2, 'FaceColor', color, 'EdgeColor', 'k', 'FaceAlpha', 1);
        ...
        % ylim([-0.5, y_lim_max]);
        ylim([-1, 1]);
        ylabel('Corrected connectivity diff index');
        legends = {'Positive', 'Negative'};
        legend(legends, 'Location', 'south', 'FontSize', 6, 'Box', 'off');
        title([area_type_names{area_type_idx}, ', Saline corrected index']);
