function correlogram_session_targetted(session_type, session_idx, target, normalization)
...
% plotting for targetted neuron pair

tic;

area_i = target.area_i;
id_i = target.id_i;
area_j = target.area_j;
id_j = target.id_j;

i = find(strcmp(cell_area, area_i) & strcmp(cell_id, id_i));
j = find(strcmp(cell_area, area_j) & strcmp(cell_id, id_j));
...
% global title
sgtitle([session_type, ', session ', int2str(session_idx), ', ', cell_area{i}, ' ', cell_id{i}, ...
    ' vs ', cell_area{j}, ' ', cell_id{j}], 'FontSize', 24);

% save figure
fig_folder = ['../../figures/correlograms/targetted/', session_type, '_', int2str(session_idx)];
