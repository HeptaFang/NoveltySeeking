...
for control_idx = 1:length(controls)
    ...
    for session_idx = 1:session_num
        ...
        subsession_types = {...
        %     'PreDecision', 'PreInfoAnti','PreInfoResp', 'PreInfo',...
        %  'PreRestOpen', 'PreRestClose', 'PreRandomA', 'PreRandomB',...
           'PostDecision', 'PostInfoAnti', 'PostInfoResp', 'PostInfo',...
         'PostRestOpen', 'PostRestClose', 'PostRandomA', 'PostRandomB',...
            };
        for subsession_idx = 1:length(subsession_types)
            ...
            % save data
            check_path(['../GLM_data/', control, subsession, '_cortex']);
            merged_file = ['../GLM_data/', control, subsession, '_cortex/raster_', ...
                    control, subsession, '_cortex_', int2str(session_idx), '_0.mat'];
            save(merged_file, 'N', "cell_id", "cuetype", "firing_rates", "n_trial",...
                "rasters", "spikes", "session_name_full", "trial_len", "cell_area", "channel");

            save(['../GLM_data/',control, subsession, '_cortex/borders_',...
                control, subsession, '_cortex_', ...
